...
arquivo postgrsql.conf

datestyle

-- BI -> ANALISE DO PASSADO DOS DADOS
-- DATA SCIENCE -> PREVISÃO DOS DADOS

-- PARA ESSAS DUAS ÁREAS, OS VALORES TRABALHADOS
-- DEVEM PREFERENCIALMENTE SEREM AGREGADOS
-- COUNT, MAX, MIN ...
...
GROUP BY MAQUINA ORDER BY MEDIA

-- MODA DE TODAS MAQUINAS
...
-- MODA CHAT GPT
WITH ContagemQTD AS (
    SELECT MAQUINA, QTD, COUNT(*) AS FREQ
    FROM  MAQUINAS
    GROUP BY MAQUINA, QTD
),
MaxFreq AS (
    SELECT MAQUINA, MAX(FREQ) AS MAX_FREQ
    FROM ContagemQTD
    GROUP BY MAQUINA
)
SELECT CQ.MAQUINA, CQ.QTD, CQ.FREQ AS MODA
FROM ContagemQTD CQ
JOIN MaxFreq MF 
ON CQ.MAQUINA = MF.MAQUINA AND CQ.FREQ = MF.MAX_FREQ
ORDER BY CQ.MAQUINA, CQ.QTD;
...
--	CRIANDO TABELA A PARTIR DE UM SELECT
...
-- TRAZENDO APENAS REGISTROS NOVOS PARA O RELATORIO
-- VENDO OS VALORES DO ID
SELECT 	MAX(IDLOCACAO) AS RELATORIO, 
		(SELECT MAX(IDLOCACAO) AS LOCACAO FROM LOCACAO)
...
ON L.ID_FILME = F.IDFILME
WHERE L.IDLOCACAO 
NOT IN (SELECT IDLOCACAO FROM RELATORIO_LOCADORA)

-- ADICIONANDO NOVOS REGISTROS PARA TESTAR
...
-- CRIAR UMA TRIGGER PARA AUTOMATIZAR
